...
const leagueFromName = (name) => {
  ...
  if (name.includes('Champions League')) return 'UCL';
  return null;
};
...
const loadData = async () => {
  ...
  try {
    // Understat: best-effort for all supported leagues (excluding UCL as it has no Understat data)
  ...
  try {
    ...
    // Add league filter if selected (UCL ID: 2)
    const leagueParam = leagueFilter !== 'all' ? `&league=${getLeagueId(leagueFilter)}` : '';
    const fixturesResp = await proxyFetch(`/fixtures?date=${date}&status=NS${leagueParam}`);
    const oddsResp = await proxyFetch(`/odds?date=${date}${leagueParam}`);
...
const renderMatches = (fixtures, oddsList, minV=0, leagueFilter='all') => {
  ...
  if (!fixtures || fixtures.length === 0) {
    container.innerHTML = '<div class="no-data">Keine Spiele für das gewählte Datum. Probiere 18.10.2025 (Premier League) oder 21.10.2025 (Champions League).</div>';
    return;
  }
  ...
  if (aboveMin.length === 0) {
    container.innerHTML = '<div class="no-data">Keine Spiele entsprechen dem Filter. Probiere ein Datum mit Matches wie 18.10.2025 oder 21.10.2025.</div>';
    return;
  }
...
function renderSample() {
  ...
  const sampleFixtures = [{
    ...
    "fixture": { "id": 3, "date": "2025-10-20" },
    "league": { "name": "UEFA Champions League" },
    "teams": { "home": { "name": "Team UCL A", "logo": "" }, "away": { "name": "Team UCL B", "logo": "" } }
  }];
  const sampleOdds = [{
    "fixture": { "id": 1 },
    "bookmakers": [{
    ...
    "fixture": { "id": 3 },
    "bookmakers": [{
      "bets": [{ "name": "Match Winner", "values": [{"value":"Home","odd":"2.00"},{"value":"Draw","odd":"3.50"},{"value":"Away","odd":"3.80"}] }]
    }]
  }];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  qs('#league-select').addEventListener('change', debounce(loadData, 250));
